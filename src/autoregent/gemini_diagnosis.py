...
class _GeminiDriftResponse(BaseModel):
    ...
    mode supports it; a plain AI Studio key does not. Converted back into
    DriftDiagnosis.field_mapping (a real dict) after parsing."""
...
async def diagnose_drift(
    config: AutoregentConfig, route: str, expected_schema: dict, validation_error: str, original_payload: dict,
) -> DriftDiagnosis | None:
    ...
    try:
        ...
        log_event(logger, logging.WARNING, "gemini_timeout", route=route, timeout_seconds=config.gemini_timeout_seconds)
        return None
