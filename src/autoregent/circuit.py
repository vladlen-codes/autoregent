...
class RouteCircuit:
    ...
    def __init__(self, config: AutoregentConfig) -> None:
        self._config = config
    ...
    def _prune(self, now: float) -> None:
        window = self._config.rolling_window_seconds
    ...
    def is_window_exhausted(self) -> bool:
        now = time.monotonic()
        self._prune(now)
        return len(self._heal_timestamps) >= self._config.rolling_window_max_heals
    ...
    def allow_request(self) -> bool:
        ...
        if time.monotonic() - self.opened_at >= self._config.circuit_cooldown_seconds:
...
class CircuitRegistry:
    ...
    def get(self, route: str) -> RouteCircuit:
        if route not in self._circuits:
            self._circuits[route] = RouteCircuit(self._config)
